...
namespace Spring.Objects.Factory.Config
{
    ...
    ///         "providerName": "System.Data.SqlClient" /*Optional*/
    ///     },
    ///     "Prd": {
    ///         "connectionString": "Server=(localdb)\\MSSQLLocalDB;Database=_CHANGE_ME;Trusted_Connection=True;"
    ///     }
    ...
    [Serializable]
    public class NetCoreConnectionStringsVariableSource : IVariableSource
    {
        ...
        /// Initializes a new instance of <see cref="NetCoreConnectionStringsVariableSource"/> from the given <paramref name="sectionName"/>
        /// </summary>
        public NetCoreConnectionStringsVariableSource(IConfiguration configuration, string sectionName = null)
        {
            AssertUtils.ArgumentNotNull(configuration, "configuration");
            if (StringUtils.IsNullOrEmpty(sectionName) == false)
            {
                this.sectionName = sectionName;
            }
            IConfigurationSection connSection = configuration.GetSection(this.sectionName);
            if (connSection != null && connSection.Exists() && connSection.GetChildren().Any())
            {
                this.configurationSection = connSection;
            }
            else
            {
                throw new ArgumentNullException($"Section {sectionName} is null or children empty!");
            }
        ...
        private void InitVariables()
        {
            ...
            foreach (IConfigurationSection setting in configurationSection.GetChildren())
            {
                string settingName = setting.Key;

                variables.Add(settingName + ".connectionString", setting.GetValue<string>("connectionString"));
                variables.Add(settingName + ".providerName", setting.GetValue("providerName", "System.Data.SqlClient"));
            }
        }
    }
}
